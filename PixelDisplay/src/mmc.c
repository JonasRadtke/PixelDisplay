...
#define FASTCLK		SPI->SPI_CSR[0] = SPI_CSR_NCPHA | SPI_CSR_BITS_8_BIT | (0x03 << SPI_CSR_SCBR_Pos)  | (0x01 << SPI_CSR_DLYBCT_Pos) ;
#define SLOWCLK		SPI->SPI_CSR[0] = SPI_CSR_NCPHA | SPI_CSR_BITS_8_BIT | (0xf0 << SPI_CSR_SCBR_Pos) | (0x01 << SPI_CSR_SCBR_Pos) | (0x01 << SPI_CSR_DLYBCT_Pos) ;
...
void init_spi(){
	...
	pio_set_peripheral	( PIOA, PIO_PERIPH_A, SPIMISO | SPIMOSI | SPICLK  ); // SPI Periph A Activation
	pio_set_output 	( 	PIOA, SPINSS ,LOW,DISABLE,DISABLE); // Setze Chip Select Ausgang
...
{
	uint8_t res;
	spi_transceive_packet(SPI, &dat, &res, 1);
	return res;
}
...
int selectSPI (void)	/* 1:OK, 0:Timeout */
{
	PIOA->PIO_CODR = SPINSS;
